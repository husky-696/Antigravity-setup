# 📚 Antigravity Elite Skill Catalog

Total skills available: **16** (Optimized for Web, Mobile, Workflow Automation, and Premium UI)

Use these skills by typing `/skill-id` (e.g., `/elite-web`). Workspace templates also install slash workflows such as `/theme` and `/agent-boost` into `.agent/workflows/`.
...
| ID | Name | Description |
| :--- | :--- | :--- |
| `🚢 /elite-ship` | **elite-ship** | Vercel, EAS Build, Bun-first package management, and Repo hygiene. |
| `🌿 /git` | **git** | Automated Conventional Commits and Repo Sync. |
| `🧹 /clean` | **clean** | Project auditing, pruning, and structural optimization. |
...
### 🎨 Build a Premium Feature
1. Activate `/elite-web` or `/elite-mobile`.
2. Use the **Premium Design Rules** from the skill to draft the UI.
3. Implement with `/elite-backend` for data persistence.
4. Run `/elite-ship` when the feature is ready to release.

### 🐛 Resolve a Critical Bug
1. Activate `/debug`.
2. Follow the **Isolate → Trace → Analyze** protocol.
3. Ship the fix with `/elite-ship`.

### 🚀 Initialize New Project
1. Use `/setup` to audit the environment.
2. Run `./setup.sh --all /path/to/project` to install workspace rules, workflows, themes, and stack detection.
3. Activate necessary `/elite-*` skills based on the stack.
...
1. Use `/agent-boost-audit` to find weak rules, missing checks, or repetitive manual work.
2. Use `/agent-boost-workflow` to turn repeated tasks into slash workflows.
3. Use `/agent-boost-validate` to document the exact checks the agent should run before reporting done.
